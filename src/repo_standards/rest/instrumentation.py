...
class _ValidatedFiles(NamedTuple):
...
def _validate_files(files: tuple[TrackedFile, ...]) -> _ValidatedFiles:
    ...
    return _ValidatedFiles(tuple(sorted(files, key=lambda item: item.path)), total)
