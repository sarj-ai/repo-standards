...
_MAX_NODES = 20_000
_MAX_DEPTH = 50
_MAX_JOB_TIMEOUT = timedelta(hours=6)
...
def _parse_jobs(value: object) -> tuple[WorkflowJobInspection, ...]:
    ...
    for job_id, raw_job in sorted(value.items()):
        ...
        has_reusable_uses = "uses" in raw_job
        if has_steps == has_reusable_uses:
            WorkflowInputError.fail(
                f"workflow job {job_id!r} must contain exactly one of steps or reusable uses"
            )
        ...
        if not steps and not has_reusable_uses:
        ...
        jobs.append(
            WorkflowJobInspection(
                ...
                has_timeout=has_reusable_uses or _has_valid_timeout(job_id, raw_job),
                steps=steps,
...
def _has_valid_timeout(job_id: str, raw_job: dict[object, object]) -> bool:
    if "timeout-minutes" not in raw_job:
        return False
    timeout = raw_job["timeout-minutes"]
    if (
        isinstance(timeout, bool)
        or not isinstance(timeout, int)
        or timeout < 1
        or timedelta(minutes=timeout) > _MAX_JOB_TIMEOUT
    ):
        WorkflowInputError.fail(
            f"workflow job {job_id!r} timeout-minutes must be an integer from 1 to 360"
        )
    return True
...
def _parse_step(job_id: str, value: object) -> WorkflowStepInspection:
    ...
    if uses is not None and run is not None:
        WorkflowInputError.fail(
            f"workflow job {job_id!r} step must contain exactly one of uses or run"
        )
