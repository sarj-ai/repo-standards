...
from repo_lint_core.inspection import (
    GitIdentity,
    git_identity,
    inspect_repository,
...
from repo_lint_openapi import analysis_schema as openapi_analysis_schema
from repo_lint_openapi import analyze as analyze_openapi
from repo_lint_openapi import local_reference_paths
from repo_lint_openapi import rules as openapi_rules
from repo_lint_rest import (
...
def _envelope(
    ...
    *,
    ...
    """Build the common deterministic machine-response envelope."""
    return {
        "schema_version": 1,
        "tool": _tool(),
        "command": command,
...
def _tool() -> Mapping[str, object]:
    """Return installed-package provenance without a duplicated version constant."""
    try:
        version = metadata.version(_DISTRIBUTION_NAME)
    except metadata.PackageNotFoundError:
        version = "unknown"
    return {"name": "repo-lint", "version": version}
...
def rest_rules_command() -> None:
    ...
    payload = {
        **_envelope(
            "rest.rules",
            provenance={"kind": "installed-package", "package": "repo-lint-openapi"},
        ),
...
def print_schema(
    ...
    if document == "openapi-analysis":
        typer.echo(json.dumps(_openapi_report_schema(), indent=2, sort_keys=True))
        return
    if document != "report":
        _emit_command_error(
            "schema",
            ...
            phase="request",
            remediation="Request `report` or `openapi-analysis`.",
        )
...
def _openapi_report_schema() -> Mapping[str, object]:
    schema = dict(openapi_analysis_schema())
    raw_required = schema.get("required")
    raw_properties = schema.get("properties")
    if not _is_object_list(raw_required) or not _is_object_mapping(raw_properties):
        ConfigurationError.fail("installed OpenAPI analysis schema is incompatible")
    ...
    properties: dict[str, object] = {
        key: value for key, value in raw_properties.items() if isinstance(key, str)
    }
    properties.update(
        {
            "tool": {
...
def _report_schema() -> Mapping[str, object]:
    ...
    raw_properties = schema.get("properties")
    if not _is_object_list(raw_required) or not _is_object_mapping(raw_properties):
        ConfigurationError.fail("installed report schema is incompatible")
    required = [item for item in raw_required if isinstance(item, str)]
...
def _page_options(limit: str, cursor: str | None) -> tuple[int, int]:
    try:
