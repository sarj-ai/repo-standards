...
from repo_lint.github import WorkflowDocument, inspect_workflow, inspect_workflows
...
@pytest.mark.parametrize(
    "job",
    [
        "steps:\n      - run: 'true'\n    uses: acme/reuse/.github/workflows/ci.yml@main",
        "steps:\n      - uses: actions/checkout@v4\n        run: echo invalid",
    ],
)
def test_rejects_mutually_exclusive_workflow_job_and_step_shapes(job: str) -> None:
    content = f"on: push\njobs:\n  invalid:\n    {job}\n".encode()
...
@pytest.mark.parametrize("timeout", ["null", "true", "0", "-1", "361", "'5'"])
def test_rejects_invalid_job_timeout_minutes(timeout: str) -> None:
    content = f"""on: push
jobs:
  test:
...
""".encode()
    ...
    with pytest.raises(ValueError, match="timeout-minutes"):
        inspect_workflow(WorkflowDocument(".github/workflows/ci.yml", content))
