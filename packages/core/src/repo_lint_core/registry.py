"""Deterministic registry for trusted, installed repository policies."""
...
class PolicyRegistry:
    """Explicit immutable set of compatible policy providers."""

    policies: tuple[Policy, ...]

    @classmethod
    def from_installed(cls) -> PolicyRegistry:
        """Load policies only from the pinned tool environment."""
        loaded: list[Policy] = []
        for entry in sorted(
    ...
    def resolve(self, policy_id: str) -> Policy:
        """Resolve one exact installed policy ID."""
    ...
    def policy_ids(self) -> tuple[PolicyId, ...]:
        """Return installed IDs in deterministic order."""
