...
import pytest
from repo_lint_github import GitHubClient
...
class FakeTransport:
    ...
    def __call__(self, request: Request, timeout: timedelta) -> tuple[int, bytes]:
    ...
    class MissingFullNameTransport(FakeTransport):
        def __call__(self, request: Request, timeout: timedelta) -> tuple[int, bytes]:
            if request.full_url.endswith("/repos/acme/widgets"):
                self.requests.append(request)
                return 200, b'{"default_branch":"main","allow_auto_merge":true}'
...
def test_network_failures_become_execution_issues_without_leaking_details() -> None:
    def fail(request: Request, timeout: timedelta) -> tuple[int, bytes]:
        del timeout
...
def test_permissions_403_marks_evidence_incomplete() -> None:
    def transport(request: Request, timeout: timedelta) -> tuple[int, bytes]:
        del timeout
        ...
        if "/branches?" in request.full_url or "/rulesets?" in request.full_url:
            return 200, b"[]"
        return 200, b'{"default_branch":"main","allow_auto_merge":false,"full_name":"acme/widgets"}'
...
def test_realistic_ruleset_summaries_are_resolved_in_stable_id_order() -> None:
    ...
    def transport(request: Request, timeout: timedelta) -> tuple[int, bytes]:
        ...
        return 200, b'{"default_branch":"main","allow_auto_merge":true,"full_name":"acme/widgets"}'
...
def test_malformed_decision_fields_are_unknown_and_incomplete() -> None:
    def transport(request: Request, timeout: timedelta) -> tuple[int, bytes]:
        del timeout
        if "/branches?" in request.full_url:
            return 200, b'[{"name":"main","protected":"yes"}]'
        if "/rulesets?" in request.full_url:
            return 200, b'[{"name":"missing-id"}]'
        if request.full_url.endswith("/actions/permissions/workflow"):
            return 200, b'{"default_workflow_permissions":"owner"}'
        return 200, b'{"default_branch":7,"allow_auto_merge":"yes","full_name":"acme/widgets"}'
...
def test_ruleset_detail_failure_is_partial_not_an_empty_success() -> None:
    def transport(request: Request, timeout: timedelta) -> tuple[int, bytes]:
        ...
        if "/branches?" in request.full_url:
            return 200, b"[]"
        if "/rulesets?" in request.full_url:
            return 200, b'[{"id":42,"name":"protected","enforcement":"active"}]'
