"""Conservative, non-executing inspection of supplied GitHub workflow bytes."""
...
class WorkflowInputError(ValueError):
    """Workflow input falls outside the bounded inert inspection contract."""

    @classmethod
    def fail(cls, message: str) -> NoReturn:
        """Raise a consistently constructed input error."""
        raise cls(message)
...
def inspect_workflow(  # ruff: ignore[too-many-locals] - one bounded parse pass assembles facts
    ...
    """Return deterministic facts without resolving or executing workflow expressions."""
    ...
    trigger_names, push_branches, push_uses_branches_ignore = _parse_triggers(root.get("on"))
    ...
    return WorkflowInspection(
        ...
        has_pull_request_trigger="pull_request" in trigger_names,
        has_merge_group_trigger="merge_group" in trigger_names,
        has_push_trigger="push" in trigger_names,
        has_schedule_trigger="schedule" in trigger_names,
        has_workflow_dispatch_trigger="workflow_dispatch" in trigger_names,
        ...
        push_uses_branches_ignore=push_uses_branches_ignore,
        jobs=jobs,
    )


def inspect_workflows(documents: tuple[WorkflowDocument, ...]) -> tuple[WorkflowInspection, ...]:
    """Inspect workflows in stable path order and reject duplicate paths."""
...
def _parse_triggers(value: object) -> tuple[frozenset[str], tuple[str, ...], bool]:
    ...
    return frozenset(value), branches, isinstance(push, dict) and "branches-ignore" in push
...
def _permissions_are_explicit_and_bounded(value: object) -> bool:
    """Recognize explicit permission syntax without claiming semantic minimality."""
    if value == "read-all":
        return True
    if value == {}:
        return True
...
def _action_references(
    ...
    for job in jobs:
        for value in (job.reusable_uses, *(step.uses for step in job.steps)):
            ...
            line, next_start = _uses_line(text, value, starts_by_value.get(value, 0))
            starts_by_value[value] = next_start
            immutable = (
                _OCI_DIGEST.fullmatch(reference) is not None
                if value.startswith("docker://")
                else _FULL_SHA.fullmatch(reference) is not None
            )
            output.append(ActionReference(value, line, immutable))
    return tuple(output)


def _uses_line(text: str, value: str, start: int) -> tuple[int, int]:
