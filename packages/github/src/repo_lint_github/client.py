"""Bounded, read-only GitHub REST evidence collection."""
...
import json
import re
from typing import NoReturn, cast  # ruff: ignore[banned-api] - checked before narrowing
from urllib.error import HTTPError, URLError
from urllib.parse import SplitResult, quote, urlencode, urlsplit
from urllib.request import HTTPRedirectHandler, Request, build_opener

from repo_lint_core import ExecutionIssue, JSONValue

from .models import BranchEvidence, RepositoryEvidence, RulesetEvidence


type JsonObject = dict[str, JSONValue]
type Transport = Callable[[Request, timedelta], tuple[int, bytes]]
...
class GitHubClientError(ValueError):
    """Invalid bounded client input or response."""

    @classmethod
    def fail(cls, message: str) -> NoReturn:
        """Raise a bounded client error without duplicating construction policy."""
        raise cls(message)


class GitHubResponseTypeError(TypeError):
    """GitHub returned a JSON value of the wrong container type."""

    @classmethod
    def fail(cls, message: str) -> NoReturn:
        """Raise a stable response-shape error."""
        raise cls(message)


class _SameOriginRedirectHandler(HTTPRedirectHandler):
    """Follow GitHub redirects only when credentials stay on the same origin."""
...
def _stdlib_transport(request: Request, timeout: timedelta) -> tuple[int, bytes]:
...
class GitHubClient:
    """Minimal GitHub REST client which exposes no mutating operations."""
    ...
    def collect(  # ruff: ignore[too-many-locals] - explicit evidence completeness by phase
        ...
        """Collect bounded repository metadata, branches, rulesets, and Actions policy."""
    ...
    def _ruleset_details(
        ...
        """Resolve bounded summary IDs to the detail schema containing rules."""
        ids: set[int] = set()
        complete = True
    ...
    def _request(self, path: str, params: dict[str, str] | None = None) -> bytes:
        ...
        headers: dict[str, str] = {
            ...
            "User-Agent": "repo-lint-github/0.1",
