"""Deterministic GitHub delivery and repository-governance policy analysis."""
...
from repo_lint_core import DeliveryConfig
from repo_lint_core.canonical import with_fingerprint
...
def analyze(
    ...
    *,
    ...
    """Analyze supplied evidence; never fetch, execute, or mutate target content."""
...
def _branch_names(config: DeliveryConfig | None) -> tuple[str, str, str]:
    if config is None:
        return ("main", "preview", "dev")
    return (config.production_branch, config.preview_branch, config.development_branch)
