"""Bounded JSON OpenAPI analysis with no filesystem, network, or subprocess access."""
...
class _InputError(ValueError):
    ...
    def fail(cls, message: str, *, cause: Exception | None = None) -> NoReturn:
        """Raise a consistently constructed bounded-input failure."""
        if cause is None:
            raise cls(message)
...
def analyze_bytes(
    ...
    *,
    ...
    """Analyze one entry document and optional in-memory local-reference targets."""
...
def local_reference_paths(name: str, content: bytes) -> tuple[str, ...]:
    """Return bounded, syntactically local document paths referenced by supplied JSON bytes."""
...
def analyze(request: AnalysisRequest) -> AnalysisReport:
    """Analyze supplied bytes without filesystem, network, mutation, or command execution."""
...
def _walk(value: JsonValue, pointer: str = "") -> list[tuple[str, JsonValue]]:
    result: list[tuple[str, JsonValue]] = [(pointer, value)]
...
def _reference_target(source_name: str, reference: str) -> tuple[str, str] | None:
    try:
    ...
    try:
        ...
        return None
    return target_name, unquote(split.fragment)
...
def _resolve_pointer(root: JsonValue, fragment: str) -> JsonValue | None:
    ...
    for raw_token in fragment[1:].split("/"):
        ...
        if isinstance(current, dict) and token in current:
            ...
            and len(token) <= _MAX_POINTER_INDEX_LENGTH
            and token.isdigit()
...
def _operations(root: JsonObject) -> list[tuple[str, str, JsonObject, JsonObject]]:
    paths = root.get("paths")
    if not isinstance(paths, dict):
        return []
    result: list[tuple[str, str, JsonObject, JsonObject]] = []
...
def _server_objects(root: JsonObject) -> list[tuple[str, JsonObject]]:
    result: list[tuple[str, JsonObject]] = []
    servers = root.get("servers")
    if isinstance(servers, list):
        for index, server in enumerate(servers):
            if isinstance(server, dict):
                result.append((f"/servers/{index}", cast("JsonObject", server)))
    for _method, pointer, operation, path_item in _operations(root):
        for owner_pointer, owner in ((pointer.rsplit("/", 1)[0], path_item), (pointer, operation)):
            ...
            for index, server in enumerate(local_servers):
                if isinstance(server, dict):
                    result.append((f"{owner_pointer}/servers/{index}", cast("JsonObject", server)))
    return result
...
def _operation_map(root: JsonObject) -> dict[str, tuple[str, JsonObject]]:
    return {
        f"#{pointer}": (method, operation) for method, pointer, operation, _ in _operations(root)
    }
...
def _artifact_finding(  # ruff: ignore[too-many-arguments, too-many-positional-arguments] - rule-specific diagnostic adapter
    ...
    collector.finding(
        rule_id="rest/artifact/provenance",
        severity=severity,
        message=message,
        observed=observed,
        expected=expected,
