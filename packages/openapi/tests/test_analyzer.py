"""Labeled evaluation cases for objective OpenAPI rules."""
...
import pytest
from repo_lint_openapi import AnalysisReport, DocumentInput, analyze_bytes, rules
...
def test_artifact_provenance_missing_and_mismatch_are_distinct() -> None:
    ...
    findings = [item for item in report.diagnostics if item.rule_id == "rest/artifact/provenance"]
...
def test_rule_catalog_is_complete_unique_and_source_backed() -> None:
    ...
    assert len({item.rule_id for item in catalog}) == len(catalog) == 10
    assert all(item.problem and item.harm and item.references for item in catalog)
